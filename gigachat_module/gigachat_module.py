...
class GigaChatAPI:
    ...
    def generate_image(
        ...
        # Используем короткий и простой промпт
        image_prompt = f"Нарисуй {prompt}"
        if negative_prompt:
            image_prompt = f"Нарисуй {prompt}, без {negative_prompt}"
        ...
        try:
            if hasattr(response, 'choices') and len(response.choices) > 0:
                ...
                raise Exception(
                    ...
                    f"Попробуйте использовать Kandinsky API для генерации изображений."
                )
            else:
